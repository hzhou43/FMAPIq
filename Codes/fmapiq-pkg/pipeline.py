...
class Pipeline:
    ...
    def stage1(self, pqr_file, ion, temp, escl, vscl, threads):
        """Stage 1: Prepare energy maps"""
        if not (pqr_file or self.pqr_file):
            return "Error: No PQR file provided"
        
        # Update configuration
        self.update_config(ion=ion, temp=temp, escl=escl, vscl=vscl)
        ...
        try:
            ...
            try:
                ...
                with open('fmap.ern', 'w') as temp_file:
                    ...
                    try:
                        result = fmap_main(['subA.vdw', 'parms.txt', ANGLE_DATA])
                        if result is not None and result != 0:
...
def main():
    ...
    if stage1_needed:
        ...
        result = pipeline.stage1(pqr_file, args.ion, args.temp, args.escl, args.vscl, pipeline.threads)
        print(result)
        if "Error" in result:
            return False
    else:
