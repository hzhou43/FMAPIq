...
def create_gui():
    ...
    def run_stage1(pqr_file, ion, temp, nrot, escl, vscl, threads_val, update_only):
        ...
        if update_only:
            ...
            result = pipeline.stage1(pqr_file.name, ion, temp, escl, vscl, int(threads_val), int(nrot))
            return f"{auto_msg}{result}", get_session_display()
        elif pipeline.pqr_file:
            result = pipeline.stage1(None, ion, temp, escl, vscl, int(threads_val), int(nrot))
            return f"{auto_msg}{result}", get_session_display()
    ...
    with gr.Blocks(title="FMAPIq Pipeline") as app:
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    with gr.Row():
                        ...
                        nrot = gr.Dropdown(choices=[72, 576, 4608], value=72, label="nrot")
                    with gr.Row():
        ...
        stage1_btn.click(run_stage1,
                      [pqr_file, ion_strength, temperature, nrot, escl, vscl, threads, update_only_1],
...
def main(share=False):
    ...
    app = create_gui()
    app.queue()
    app.launch(share=share, show_api=False, show_error=True)
